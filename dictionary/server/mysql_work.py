...
class Mysql_opt :
    ...
    def __init__(self):
        ...
        self.__conn = pymysql.connect(**self.settings.mysql_connection_opt)
    ...
    def __init_database(self):
        build_database = "create database if not exists %s;" %self.settings.mysql_database_name
        select_database = "use %s" %self.settings.mysql_database_name
    ...
    def __init_word_book(self):
        build_table_dictinfo = "create table if not exists %s ( id int auto_increment primary key, \
                                                                        ...
                                                                        meanings varchar(256) );" % self.settings.mysql_table_dict
        self.__cur.execute(build_table_dictinfo)

    def __dict_book(self):
        self.__cur.execute("select word from %s limit 3"%self.settings.mysql_table_dict)
        tables = self.__cur.fetchall()
        if tables :
            return
        self.settings.logger.warning("init database and dict table...")
        ...
        with open(r"./dict.txt") as f:
            for line in f:
                ...
                sql = "insert into %s(word,meanings) values (%r,%r);" % (self.settings.mysql_table_dict, w, m)
                try :
                    self.__cur.execute(sql)
                except Exception as e:
                    self.settings.logger.warning("%s is %s" % (line, e))
            self.__conn.commit()
        self.settings.logger.warning("init done...")

    def __init_user(self):
        build_table_user = "create table if not exists %s (id int auto_increment primary key, \
                                                            name varchar(30) unique, \
                                                            passwd varchar(128));" % self.settings.mysql_table_user
    ...
    def __init_history(self):
        build_table_history = "create table if not exists %s (id int auto_increment primary key, \
                                                                    name varchar(30), \
                                                                    word varchar(30), \
                                                                    meanings varchar(256),\
                                                                    his_date timestamp, \
                                                                    index(name));" % self.settings.mysql_table_history
    ...
    def insert_info(self, sql):
        try :
            ...
            self.settings.logger.warning("%s插入失败" % sql)
            return 0
        return 1
    ...
    def select_info(self, sql):
        try :
            ...
            self.settings.logger.warning("%s查询失败" % sql)
            return 0
